  ...
  useEffect(() => {
    ...
    const loadBlogs = async () => {
      try {
        ...
        console.error('Failed loading Hindi blogs:', err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-[var(--bg-color)] py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              BPSC एवं राज्य सिविल सेवा परीक्षाओं हेतु विशेष रणनीति, सामयिकी तथा मॉडल उत्तर।
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogsList.map((post) => {
              const displayTitle = post.title_hi || post.title;
              const displayBlurb = post.blurb_hi || post.blurb || stripHtml(post.content_hi || post.content);
