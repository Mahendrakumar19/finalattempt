...
import {
  Award, Timer, Layers, CheckCircle2, XCircle, Clock,
  ArrowRight, BookOpen, Sparkles, BarChart2, Play, RefreshCw, FileText, CheckCircle
} from 'lucide-react';
import { useAuth } from '@/hooks/useAuth';
import { db, ExamData, TestSeriesItem } from '@/services/db';
import { getMyQuizAttempts, getStudentAnalytics } from '@/services/auth';
...
export default function StudentPrelimsPage() {
    ...
    const loadHierarchy = async () => {
      try {
        ...
        if (exList && Array.isArray(exList)) {
          // Filter out exams that do not have stages (e.g. APSSB)
          const prelimsExams = exList.filter(ex => ex.hasStages !== false);
          setExams(prelimsExams);
        }
        const series = await db.getTestSeries(false);
        if (series && Array.isArray(series)) {
          // Filter for Prelims category series
          const prelimsSeries = series.filter(s => 
            !s.category || s.category.toLowerCase().includes('prelims') || true
          );
          setTestSeriesList(prelimsSeries as any);
        }
      } catch (err) {
  ...
  // Load Quizzes for selected series or all series
  useEffect(() => {
    const loadQuizzes = async () => {
      setLoadingTests(true);
      try {
        if (selectedSeriesId !== 'ALL') {
          const qList = await db.getTestSeriesQuizzes(selectedSeriesId);
          setQuizzes(qList || []);
        } else {
          // Fetch quizzes across all test series
          const allQuizzes: QuizItem[] = [];
          for (const s of testSeriesList) {
            if (s.id) {
              const qList = await db.getTestSeriesQuizzes(s.id);
  ...
  const loadAttempts = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        // Derive Real Performance Analytics
        const list = res.data;
        if (list.length > 0) {
          const totalAttempts = list.length;
          const totalScore = list.reduce((acc: number, item: any) => acc + (item.score || 0), 0);
          const totalMax = list.reduce((acc: number, item: any) => acc + (item.maxScore || 150), 0);
          const passedCount = list.filter((item: any) => item.passed).length;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--card-border)] pb-8">
          <div className="space-y-1">
            <span className="px-3.5 py-1 bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 rounded-full text-xs font-black uppercase tracking-widest inline-block">
              State Civil Services CBT Practice Vault
            </span>
            <h1 className="text-3xl sm:text-4xl font-heading font-black text-[var(--text-color)] tracking-tight">
              Prelims Test Series &amp; CBT Portal
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-2xl leading-relaxed">
              Computer-Based Tests (CBT), Exam Pattern Mock Papers, Real-Time Scorecard, and Performance Analytics across State Civil Services.
            </p>
          </div>
        ...
        {analytics.totalAttempts > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-5 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl space-y-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Tests Attempted</span>
              <p className="text-2xl font-black text-[var(--text-color)]">{analytics.totalAttempts}</p>
              <p className="text-[11px] text-slate-500 font-semibold">CBT Mocks Completed</p>
            </div>

            <div className="p-5 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl space-y-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Average Score</span>
              <p className="text-2xl font-black text-amber-500">{analytics.avgScore.toFixed(1)} <span className="text-xs text-slate-400 font-normal">Marks</span></p>
              <p className="text-[11px] text-slate-500 font-semibold">Mean Performance</p>
            </div>
            ...
            <div className="p-5 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl space-y-2">
              ...
              <p className="text-[11px] text-slate-500 font-semibold">Scoring Efficiency</p>
            </div>

            <div className="p-5 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl space-y-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Cutoff Pass Rate</span>
              <p className={`text-2xl font-black ${analytics.passRate >= 50 ? 'text-emerald-500' : 'text-amber-500'}`}>
                {analytics.passRate.toFixed(1)}%
              </p>
              <p className="text-[11px] text-slate-500 font-semibold">Qualification Ratio</p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          {loadingTests ? (
            ...
            <div className="p-12 text-center bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl space-y-3">
              ...
              <h3 className="text-[var(--text-color)] font-bold text-base">No Prelims CBT Tests Available</h3>
              <p className="text-slate-400 text-xs max-w-sm mx-auto">
                No active CBT tests match the selected filter.
              </p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="pt-4 border-t border-[var(--card-border)] flex items-center justify-between gap-3">
                      ...
                      >
                        ...
                        {userAttempt ? 'Retake CBT Test' : 'Start CBT Test'}
                      </Link>
                    </div>
                  </div>
                );
