...
function BlogEditorForm() {
          ...
          if (Array.isArray(list)) {
            ...
            if (post) {
              setFormData({
                ...
                author_name: post.author_name || 'Final Attempt IAS',
                ...
                publishDate: post.publishDate || new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }),
  ...
  const handleSubmit = async (e: React.FormEvent, isPublishing: boolean = false) => {
    ...
    try {
      ...
      const url = postId ? `${BACKEND_URL}/api/blogs/${postId}` : `${BACKEND_URL}/api/blogs`;
  ...
  if (fetching) return <div className="p-8 text-center text-slate-500 font-semibold animate-pulse">Loading blog post data...</div>
            ...
            <div className="space-y-2">
              ...
              {formData.imageUrl && (
                <div className="relative mt-3 rounded-xl overflow-hidden border border-border/60 aspect-video group">
                  <img src={formData.imageUrl} alt="Cover Preview" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                </div>
              )}
            </div>
          </div>
