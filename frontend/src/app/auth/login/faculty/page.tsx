...
import { z } from 'zod';
import { Eye, EyeOff, Mail, Lock, Sparkles, ArrowRight, Phone, BookOpen, GraduationCap, Trophy, Target, Star, ShieldCheck } from 'lucide-react';
...
function LoginFormContent() {
  ...
  const redirectTo = searchParams.get('redirect') || '/faculty/dashboard';
  ...
  const handlePasswordLogin = async (data: LoginForm) => {
    ...
    if (res.success && res.data) {
      // Allow both faculty and admin role login in this page
      ...
      if (role === 'admin' || role === 'faculty') {
        ...
        setError('This portal is reserved for Faculty and Administrators only.');
      }
    } else {
  ...
  return (
    <div className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-2 gap-0 rounded-3xl overflow-hidden shadow-2xl border border-white/10 relative z-10"
      ...
      <div className="hidden lg:flex flex-col justify-between relative overflow-hidden p-10"
        style={{ background: 'linear-gradient(135deg, rgba(15,23,42,0.95) 0%, rgba(20,83,45,0.92) 50%, rgba(15,23,42,0.95) 100%)' }}>

        <div className="absolute top-0 right-0 w-64 h-64 rounded-full opacity-20 pointer-events-none"
          style={{ background: 'radial-gradient(circle, #22c55e 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />

        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl border border-emerald-500/25 bg-emerald-500/10">
      ...
      <div className="flex flex-col justify-center p-8 sm:p-10"
        ...
        <div className="flex gap-1 p-1 rounded-xl mb-6 border border-white/[0.08]"
          ...
          <button
            ...
            style={mode === 'password' ? { background: 'linear-gradient(135deg, #10B981, #059669)' } : {}}
          >
            Password Login
          </button>
          <button
          ...
          <form onSubmit={loginForm.handleSubmit(handlePasswordLogin)} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onFocus={e => { e.currentTarget.style.border = '1px solid rgba(16,185,129,0.7)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(16,185,129,0.12)'; }}
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                <Link href="/auth/forgot-password" className="text-[10px] text-emerald-500 hover:text-emerald-450 font-medium transition-colors">
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #059669 0%, #10B981 100%)', boxShadow: '0 4px 24px rgba(5,150,105,0.35)' }}
            >
              {isSubmitting ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><span>Sign In as Faculty</span><ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>
        )}
        ...
        {mode === 'otp-send' && (
          <div className="space-y-4">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #059669, #10B981)', boxShadow: '0 4px 24px rgba(5,150,105,0.35)' }}
            >
        ...
        {mode === 'otp-verify' && (
          <form onSubmit={otpForm.handleSubmit(handleVerifyOTP)} className="space-y-4">
            <div className="p-3 rounded-xl text-emerald-300 text-xs border border-emerald-500/20"
        ...
        <Link href="/" className="mt-8 text-center block text-slate-500 hover:text-slate-400 text-xs transition-colors">
          ← Back to main site
        </Link>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, #020617 0%, #0F172A 35%, #052e16 65%, #0F172A 85%, #020617 100%)' }}>
