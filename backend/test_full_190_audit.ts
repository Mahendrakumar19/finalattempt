...
async function audit190() {
  ...
  const doc = await AdapterFactory.process(buffer, { filename: 'FULL_190.txt', mimeType: 'text/plain' });
  const qnas = await QnaExtractor.extractQna(doc);

  console.log(`\n====================================================`);
  console.log(`FULL 190 AUDIT: Extracted ${qnas.length} total questions.`);
  console.log(`====================================================\n`);

  const presentNums = new Set(qnas.map(q => q.questionNumber));
  const missingNums: number[] = [];

  for (let num = 1; num <= 190; num++) {
  ...
  if (missingNums.length > 0) {
    console.error(`❌ MISSING QUESTIONS DETECTED: (${missingNums.length} total missing)`);
    console.error(`   Missing Numbers:`, missingNums.join(', '));
  } else {
    console.log(`✅ ZERO QUESTIONS SKIPPED! All questions 1 to 190 extracted with 100% precision!`);
  }
}
